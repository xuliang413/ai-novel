...
> `currentGoal`、`currentEmotion` 为**动态摘要属性**，每次章节更新后同步修改。`currentLocation` 不作为事实源；角色当前位置统一以 `(:Character)-[:CURRENTLY_AT]->(:Location)` 为准，避免属性和关系出现冲突。
...
| `KNOWS_ABOUT` | Character | Clue | 角色知情某条线索（属性 `level`: 0.0~1.0） |
...
| ③ | 候选角色完整状态卡 | `ChapterIntent.candidateCharacters` + `Character` + `CURRENTLY_AT` | 图查询 | 5 人 |
| ④ | 候选角色简短提示 | 排名 6~10 的候选角色 | 图查询 | 5 人 |
| ⑤ | 关键人物关系 | 候选角色之间的 `KNOWS / LOVES / HATES / IS_FAMILY_OF` | 图查询 | 只保留冲突/驱动剧情关系 |
| ⑥ | 目标线索完整进展 | `ChapterIntent.targetClues` | 图查询 | 3 条 |
| ⑦ | 关联线索简短提示 | 与目标线索 `INTERSECTS` 或共享角色的线索 | 图查询 | 3 条 |
| ⑧ | 金手指 + 马甲 | `Cheat` + `Alias` WHERE 属于候选角色 | 图查询 | 只取候选角色相关 |
| ⑨ | 主地点 + 相关地点 | `ChapterIntent.candidateLocations` + POV 当前所在地 + 前章结尾地点 | 图查询 | 1 完整 + 2 简短 |
...
| 分组 | 操作 | 对应节点/关系 |
|------|------|-------------|
| 结构 | `CREATE_VOLUME`、`UPDATE_VOLUME`、`CREATE_CHAPTER`、`UPDATE_CHAPTER_SUMMARY`、`LINK_CHAPTER_PREVIOUS`、`ATTACH_RULE` | `Volume`、`Chapter`、`CONTAINS`、`PREVIOUS`、`HAS_RULE` |
| 角色 | `CREATE_CHARACTER`、`UPDATE_CHARACTER_STATE`、`MOVE_CHARACTER`、`UPDATE_CHARACTER_RELATION`、`MARK_CHARACTER_STATUS` | `Character`、`CURRENTLY_AT`、`KNOWS`、`LOVES`、`HATES`、`IS_FAMILY_OF` |
| 地点 | `CREATE_LOCATION`、`UPDATE_LOCATION`、`MARK_LOCATION_APPEARANCE` | `Location`、`APPEARS_IN` |
| 物品 | `CREATE_ITEM`、`UPDATE_ITEM_STATUS`、`TRANSFER_ITEM`、`MARK_ITEM_APPEARANCE` | `Item`、`POSSESSES`、`APPEARS_IN` |
| 事件 | `CREATE_EVENT`、`UPDATE_EVENT`、`MARK_EVENT_OCCURRED`、`LINK_EVENT_PARTICIPANT` | `Event`、`APPEARS_IN`、`PARTICIPATES_IN` |
| 线索 | `CREATE_CLUE`、`UPDATE_CLUE`、`ADVANCE_CLUE`、`RESOLVE_CLUE`、`LINK_CLUE_CHARACTER`、`MARK_CHARACTER_KNOWS_CLUE`、`INTERSECT_CLUES`、`TRIGGER_CLUE`、`ASSIGN_CLUE_TO_VOLUME` | `Clue`、`DRIVES`、`ADVANCES`、`INVOLVES`、`KNOWS_ABOUT`、`INTERSECTS`、`TRIGGERS`、`BELONGS_TO` |
| 金手指 | `CREATE_CHEAT`、`UPDATE_CHEAT`、`ASSIGN_CHEAT_TO_CHARACTER`、`BIND_CHEAT_TO_ITEM` | `Cheat`、`HAS_CHEAT`、`BOUND_TO` |
...
ADVANCE_CLUE
forward:
  更新 Clue.revealLevel/status/summary
  创建 Chapter -ADVANCES-> Clue
inverse:
  恢复 Clue 原 revealLevel/status/summary
...
全量支持 11 类节点 + 26 类关系，但不是所有事实都允许 LLM 从章节正文中自由抽取。

| 分级 | 来源/权限 | 范围 | 处理 |
|------|----------|------|------|
| A 类 | 系统维护，LLM 不参与 | `Project`、`Volume`、`Chapter`、`CONTAINS`、`PREVIOUS`、`HAS_RULE` | 由创建流程、章节号、规则配置维护 |
| B 类 | LLM 可建议，用户确认后写入 | 普通实体、出场、位置、持有、参与、线索推进、金手指、马甲 | 默认按置信度展示，用户确认后执行 |
...
页面对话和图谱是两个独立菜单，用户可开两个浏览器窗口分别查看。
```
**顶栏项目切换**：
...
| 🧑 角色管理 | AntDV Table + Drawer | 列表 + 详情抽屉 + 原地编辑 + 关系网 |
| 🧵 线索管理 | AntDV Table + Progress | 列表 + 进度条 + 推进历史 + 知情者 |
| 📖 章节管理 | AntDV Table + Collapse | 按卷分组的章列表，P0 只看+改标题，正文从 MySQL `chapter_content` 加载，P1 可修改 |
| 🗺️ 地点管理 | AntDV Table + Drawer | 列表 + 详情 |
| 🎁 物品/金手指/马甲 | AntDV Table + Drawer | 统一管理 |
| 📜 叙事规则 | AntDV Table + Drawer | 平台红线/字数/文风/禁止项 |
...
> 本章作为开发执行的单一事实源。第一版开发只覆盖 **M0 技术骨架** 和 **M1 安全写作闭环**，先把“能写、能审、能入图谱、能撤销、能恢复”打硬。完整 Beta、图谱可视化、移动端、导入、全量体验增强后置。

### 7.1 阶段总览

```text
M0 技术骨架可跑
  SmartAdmin 接入 → 中间件连接 → 基础表/图谱 schema → 项目/基础实体 → mock/DeepSeek 生成 → DRAFT 保存

M1 安全写作闭环
  ChapterIntent → 上下文预览 → CONTENT_REVIEW → GraphPatch → PATCH_REVIEW → APPLYING_PATCH → PUBLISHED → /undo

M2+ 后续增强
  全量管理页 → 图谱可视化 → 导出/导入 → 向量检索/rerank → 移动端 → 协作/分享
```

| 阶段 | 目标 | 可验证结果 |
|------|------|-----------|
| M0 | 技术骨架可跑 | 能创建项目和基础图谱节点，`/write 1` 生成正文并保存 `DRAFT` |
| M1 | 安全写作闭环 | 能从 `ChapterIntent` 到 `GraphPatch`，确认后写图谱并发布正文，支持 `/undo` 和恢复 |
| M2+ | 完整产品体验 | 管理页、可视化、导出、导入、向量检索、移动端等体验增强 |

### 7.2 M0：技术骨架可跑

> 目标：接入 SmartAdmin 源码和本地中间件，跑通最短写作路径。M0 不追求完整图谱智能，只要求工程骨架和第一章草稿可用。

#### 7.2.1 接入与环境

| 任务 | 说明 | 验收 |
|------|------|------|
| SmartAdmin 源码接入 | 确认前后端目录、启动命令、鉴权方式、模块结构 | 前后端能本地启动 |
| 本地中间件配置 | 连接 Docker 中的 MySQL、Neo4j、Redis | 后端健康检查能连通三类服务 |
| 配置分层 | 提供本地开发配置，避免真实 Key 写入代码 | 默认可用 mock LLM 模式 |

#### 7.2.2 数据与图谱骨架

| 任务 | 说明 | 验收 |
|------|------|------|
| MySQL 基础表 | 建立 `project`、`chapter_content`、`user_api_key`、`chapter_generation_session`、`graph_change_log` | 表结构存在，字段含状态和 JSON 扩展字段 |
| Neo4j schema | 初始化 11 类节点、26 类关系、`projectId` 约束 | 约束可重复执行，所有查询按 `projectId` 隔离 |
| 基础实体 API | 支持 Project、Character、Clue、Location 的最小创建/查询 | 可创建项目和基础图谱节点 |

#### 7.2.3 最短生成路径

| 任务 | 说明 | 验收 |
|------|------|------|
| API Key 配置 | 用户可配置 DeepSeek / 通义千问 Key；开发期可用 mock | 未配置真实 Key 时仍能用 mock 生成 |
| `/write 1` 基础命令 | 先支持结构化命令，不要求自然语言输入 | 能触发生成任务 |
| 正文草稿保存 | 生成结果写入 `chapter_content` | `status = DRAFT`，内容可重新加载 |

✅ **M0 完成标准**：能启动前后端，连通 MySQL / Neo4j / Redis，创建项目和基础实体，执行 `/write 1`，生成第一章草稿并保存为 `DRAFT`。

### 7.3 M1：安全写作闭环

> 目标：跑通可信闭环：写前意图 → 上下文 → 生成 → 内容审阅 → 图谱变更单 → 人工确认 → 写入图谱 → 发布正文 → 可撤销/可恢复。

#### 7.3.1 ChapterIntent 与上下文

| 任务 | 说明 | 验收 |
|------|------|------|
| `ChapterIntent` 构建 | 合并用户命令、细纲、前章延续、活跃线索、项目默认 | `/write N` 生成 `intent_json` |
| 上下文检索 | 按角色 5+5、线索 3+3、地点 1+2 的预算检索 | 返回 ContextPreview 和 Token 统计 |
| 上下文审阅入口 | 支持确认、补充、移除、重新检索的最小能力 | 用户确认后进入生成 |

#### 7.3.2 内容审阅

| 任务 | 说明 | 验收 |
|------|------|------|
| 生成状态记录 | `chapter_generation_session` 记录 `GENERATING / CONTENT_REVIEW` | 刷新后可恢复生成状态 |
| `CONTENT_REVIEW` | 用户可通过、重写、保存草稿 | 通过后进入 `EXTRACTING_PATCH` |
| 自动质检最小集 | 字数、视角、前章衔接、新实体提示 | 质检结果随生成结果展示 |

#### 7.3.3 GraphPatch 与确认

| 任务 | 说明 | 验收 |
|------|------|------|
| 候选事实抽取 | LLM/mock 只输出候选事实，不产 Cypher | 原始抽取结果不直接写图谱 |
| GraphPatch 生成 | 后端生成业务语义型 `GraphPatch` 和 `inversePatch` | Patch 带 `confidence / evidence / reason` |
| Patch 校验 | 标记 `READY / LOW_CONFIDENCE / CONFLICT / BLOCKED` | 冲突项不可直接执行 |
| `PATCH_REVIEW` | 展示变更摘要，支持确认更新、返回审阅、忽略低置信项 | 用户只执行勾选操作 |
...
| `UPDATE_CHARACTER_STATE` | 更新角色目标、情绪、状态 |
| `MOVE_CHARACTER` | 更新 `CURRENTLY_AT` |
| `CREATE_LOCATION` | 新建地点 |
| `CREATE_ITEM` | 新建物品 |
| `TRANSFER_ITEM` | 更新 `POSSESSES` |
| `CREATE_CLUE` | 新建线索 |
| `ADVANCE_CLUE` | 推进线索进度和摘要 |
| `MARK_APPEARANCE` | 记录角色/地点/物品/事件出场 |

#### 7.3.4 Patch 执行、撤销与恢复

| 任务 | 说明 | 验收 |
|------|------|------|
| 白名单执行器 | 将 GraphPatch 转为受控 Cypher | 不存在 raw Cypher 写入口 |
| `graph_change_log` | 写入 `patch_json`、`inverse_patch_json`、`operation_batch_id` | 每次确认更新都有日志 |
| 发布正文 | Patch 成功后 `chapter_content.status = PUBLISHED` | 正文和图谱状态一致 |
| `/undo` | 执行最近一次 `inversePatch`，默认只撤销图谱 | 图谱回退，正文不删除 |
| 关闭恢复 | 恢复 `GENERATING / CONTENT_REVIEW / PATCH_REVIEW / APPLYING_PATCH` | 刷新/关闭页面后能回到正确卡片 |

✅ **M1 完成标准**：`/write N` 能生成 `ChapterIntent` 和上下文预览；正文审阅通过后生成 GraphPatch；用户确认后图谱更新、正文发布为 `PUBLISHED`；`/undo` 可撤销最近一次图谱变更；刷新/关闭页面后能恢复中断生成、待审阅正文、待确认 GraphPatch。

### 7.4 M0+M1 暂不纳入范围

| 功能 | 处理 |
|------|------|
| 全量管理页深度打磨 | M1 只做闭环必要的最小管理入口 |
| 图谱可视化高级交互 | 后置到 M2+ |
| UniApp 移动端 | 后置到 M2+ |
| 文件导入 / 已有章节逆向导入 | 后置到 M2+ |
| 向量检索 / rerank | 预留接口，M1 不强依赖 |
| AI 生成中动态检索 | 后续迭代 |
| 协作/分享 | 后续迭代 |
...
| 结构 | 用途 |
|------|------|
| `ChapterIntent` | 写前意图，驱动上下文检索 |
| `GraphPatch` | 图谱变更单，供用户确认和后端执行 |
| `inversePatch` | `/undo` 反向变更 |
| `chapter_generation_session` | 生成、审阅、Patch、执行状态恢复 |
| `graph_change_log` | 图谱变更审计和撤销 |
| `chapter_content.status` | 草稿、待确认、待同步、已发布、中断草稿状态 |
...
|------|------|
| 启动系统 | 前后端启动成功 |
| 中间件连通 | MySQL、Neo4j、Redis 健康检查通过 |
| 创建项目 | MySQL `project` + Neo4j `Project` 节点创建成功 |
| 创建基础实体 | Character / Clue / Location 可写入 Neo4j |
| 写第一章 | `/write 1` 生成正文，保存为 `DRAFT` |

M1 验收场景：

| 场景 | 预期 |
|------|------|
| 写第 N 章 | 生成 `ChapterIntent` 和 ContextPreview |
| 内容审阅 | 正文可通过、重写、保存草稿 |
| Patch 生成 | 通过后生成业务语义型 GraphPatch |
| Patch 确认 | 用户确认后写 Neo4j 和 `graph_change_log` |
| 发布正文 | `chapter_content.status = PUBLISHED` |
| 撤销 | `/undo` 执行 `inversePatch`，正文保留 |
| 刷新恢复 | 中断生成、待审阅正文、待确认 GraphPatch 均可恢复 |

### 7.7 计划假设

- SmartAdmin 源码稍后放入 `D:\novel\ai-novel`。
- 本地 MySQL、Neo4j、Redis 已在 Docker 中可用。
- 开发阶段先支持 mock LLM；真实 DeepSeek / 通义千问 Key 通过页面配置。
- 11 类节点 + 26 类关系保留 schema 和扩展入口，但 M1 验收优先保证写作闭环可信。
